        ...
        try:
            for op, data, idx in self.raw_iter():
                if op > OP_16:
                    ...
                    op < OP_PUSHDATA1 and op > OP_0 and len(data) == 1 and data[0] <= 16
                ):
...
class CScriptWitness(ImmutableSerializable):
    ...
    def __repr__(self):
        return (
            "CScriptWitness("
            + ",".join("x('%s')" % bitcoin.core.b2x(s) for s in self.stack)
            + ")"
        )
...
def IsLowDERSignature(sig):
    ...
    s_val = list(
        struct.unpack(str(length_s) + "B", sig[6 + length_r : 6 + length_r + length_s])
    )
    ...
    return (
        CompareBigEndian(s_val, [0]) > 0
        and CompareBigEndian(s_val, max_mod_half_order) <= 0
    )
